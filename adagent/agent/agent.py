...
class Agent:
    ...
    def __init__(
        ...
        self.model = model.bind_tools(tools) #.bind_tools() 方法将Langchain工具转换为OpenAI的工具调用格式来实现。
    ...
    def has_tool_calls(self, state: AgentState) -> bool:
        ...
        """
        response = state["messages"][-1] #？需要check“message”的格式，看[-1]具体是什么？
    ...
    def execute_tools(self, state: AgentState) -> Dict[str, List[ToolMessage]]:
        ...
        for call in tool_calls:
            ...
            if call["name"] not in self.tools:
                ...
                result = self.tools[call["name"]].invoke(call["args"]) #invoke() 函数的作用是调用特定的工具或函数
