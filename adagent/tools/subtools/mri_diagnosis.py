...
def subprocess_main():
    ...
    try:
        ...
        MedicalNet.load_state_dict(torch.load(str(_MODEL_DIR / "mri_medicalnet_800.pt")), strict=False)
        MedicalNet.eval()

        ResNet50 = generate_model(50, n_input_channels=1).cuda()
        ResNet50.load_state_dict(torch.load(str(_MODEL_DIR / "mri_resnet50_800.pt")), strict=False)
        ResNet50.eval()

        ResNet34 = generate_model(34, n_input_channels=1).cuda()
        ResNet34.load_state_dict(torch.load(str(_MODEL_DIR / "mri_resnet34_800.pt")), strict=False)
        ResNet34.eval()

        ResNet18 = generate_model(18, n_input_channels=1).cuda()
        ResNet18.load_state_dict(torch.load(str(_MODEL_DIR / "mri_resnet18_800.pt")), strict=False)
        ResNet18.eval()

        # if not os.path.isfile(args.mri_path):
        #     print('MRI Image not found: ', args.mri_path)
        # assert os.path.isfile(args.mri_path)
        MRI_Image = nibabel.load(input_data['mri_path']) 
        ...
        inputs_mri = inputs_mri.to('cuda' if torch.cuda.is_available() else 'cpu')

        MedicalNet_output = nn.functional.softmax(MedicalNet(inputs_mri), dim=1).to('cpu')
        ResNet50_output = nn.functional.softmax(ResNet50(inputs_mri), dim=1).to('cpu')
        ResNet34_output = nn.functional.softmax(ResNet34(inputs_mri), dim=1).to('cpu')
        ResNet18_output = nn.functional.softmax(ResNet18(inputs_mri), dim=1).to('cpu')
