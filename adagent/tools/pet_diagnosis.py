...
class PetDiagnosisInput(BaseModel):
    """Input schema for the AD diagnosis Tool whose input is pet. Only supports NIFTI files."""
...
class PetDiagnosisTool(BaseTool):
    """
    """

    name: str = "pet_diagnosis"
    description: str = (
        "To diagnose whether a patient has Alzheimer's Disease(AD) or not based on PET file."
        "If the input is a PET file, please use this tool and input the PET file path only. Example input: {'pet_path': '/path/to/pet.nii'}"
        "If the input is a MRI file or has two different files, please use other tool."
    ...
    args_schema: Type[BaseModel] = PetDiagnosisInput
    device: str = "cuda"
    ...
    def _run(
        ...
        try:
            ...
            # new_env['PATH'] = '/data/Data2/wlhou/anaconda3/envs/ResNet/bin:' + new_env['PATH']  # 修改 PATH 环境变量
            ...
            proc = subprocess.Popen(
                ["python", str(_SUBTOOLS_DIR / "pet_diagnosis.py")],
    ...
    async def _arun(
        ...
        return self._run(pet_path, run_manager)